                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($items as $item)
                        ...
                        <tr class="transition hover:bg-white/[.025]">
                            ...
                            <td data-label="Usuario" class="px-4 py-3"><p class="font-semibold text-white">{{ $item->usuario?->name ?? 'Usuario eliminado' }}</p><p class="mt-1 text-xs text-slate-500">{{ $item->usuario?->email ?? '—' }}</p>@if($item->usuario?->is_demo || in_array($item->usuario?->data_origin, ['test','simulated'], true))<span class="mt-1 inline-block rounded bg-violet-400/10 px-1.5 py-0.5 text-[9px] font-black uppercase text-violet-300">Demo</span>@endif</td>
