...
<div class="mx-auto max-w-[1500px] space-y-6">
    <header class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">Consulta cada apertura demo registrada y si la recompensa virtual continúa en el inventario o ya fue activada.</p>
        </div>
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <article class="rounded-2xl border border-cyan-400/15 bg-cyan-400/[.05] p-4"><p class="text-[10px] font-black uppercase tracking-wider text-cyan-300">Ingresos demo</p><p class="mt-2 text-2xl font-black text-cyan-100">{{ number_format($revenue, 2, ',', '.') }} EUR Demo</p><p class="mt-1 text-xs text-slate-500">Coste total de cajas virtuales</p></article>
        <article class="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-4"><p class="text-[10px] font-black uppercase tracking-wider text-emerald-300">Activado</p><p class="mt-2 text-2xl font-black text-emerald-100">{{ number_format($redeemed, 0, ',', '.') }} puntos</p><p class="mt-1 text-xs text-slate-500">Recompensas virtuales usadas</p></article>
        <article class="rounded-2xl border border-amber-300/15 bg-amber-300/[.05] p-4"><p class="text-[10px] font-black uppercase tracking-wider text-amber-200">Pendiente</p><p class="mt-2 text-2xl font-black text-amber-100">{{ number_format($pending, 0, ',', '.') }} puntos</p><p class="mt-1 text-xs text-slate-500">Cosméticos aún disponibles</p></article>
        <article class="rounded-2xl border p-4 {{ $margin >= 0 ? 'border-violet-400/15 bg-violet-400/[.05]' : 'border-red-400/20 bg-red-400/[.06]' }}"><p class="text-[10px] font-black uppercase tracking-wider {{ $margin >= 0 ? 'text-violet-300' : 'text-red-300' }}">Margen demo</p><p class="mt-2 text-2xl font-black {{ $margin >= 0 ? 'text-violet-100' : 'text-red-200' }}">{{ number_format($margin, 2, ',', '.') }} EUR Demo</p><p class="mt-1 text-xs text-slate-500">Los cosméticos no generan pagos</p></article>
    </section>
    ...
    <section class="overflow-hidden rounded-2xl border border-white/10 bg-white/[.025]">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
            ...
            <p class="rounded-lg bg-white/5 px-3 py-2 text-xs text-slate-400">Puntos virtuales entregados: <b class="text-white">{{ number_format((float) $summary->awarded_value, 0, ',', '.') }}</b></p>
        </div>
        <div class="admin-table-wrap overflow-x-auto">
            <table class="admin-table w-full min-w-[1080px] text-left text-sm">
                <thead class="bg-black/20 text-[10px] uppercase tracking-wider text-slate-500"><tr><th class="px-5 py-3">ID / Fecha</th><th class="px-4 py-3">Usuario</th><th class="px-4 py-3">Caja</th><th class="px-4 py-3">Recompensa</th><th class="px-4 py-3">Rareza</th><th class="px-4 py-3 text-right">Coste demo</th><th class="px-4 py-3 text-right">Puntos virtuales</th><th class="px-4 py-3">Estado</th></tr></thead>
                <tbody class="divide-y divide-white/5">
                    @forelse($items as $item)
                        <tr class="transition hover:bg-white/[.025]">
                            ...
                            <td data-label="Coste demo" class="px-4 py-3 text-right tabular-nums text-slate-300">{{ number_format($item->precio_caja, 2, ',', '.') }} EUR Demo</td>
                            <td data-label="Puntos virtuales" class="px-4 py-3 text-right font-bold tabular-nums text-white">{{ number_format($item->valor_virtual ?: round($item->valor_canje * 100), 0, ',', '.') }}</td>
