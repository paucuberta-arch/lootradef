    ...
    <div class="grid gap-6 xl:grid-cols-[1fr_330px]">
        ...
        <aside class="space-y-5">
            <section class="rounded-2xl border border-white/10 bg-white/[.035] p-5"><p class="text-[10px] font-black uppercase tracking-[.2em] text-brand-400" x-text="hand&&!finished?'Decisión actual':'Nueva partida'"></p><h2 class="mt-1 text-lg font-black" x-text="instruction"></h2>
                <template x-if="!hand||finished"><div class="mt-4"><label class="text-xs text-slate-500">Ante inicial (EUR Demo)</label><div class="mt-2 flex items-center rounded-xl border bg-black/20 px-3" :class="canStart?'border-white/10':'border-red-400/30'"><span>EUR Demo</span><input x-model.number="ante" :disabled="busy" type="number" min="1" max="1000" step="1" class="w-full bg-transparent px-3 py-3 font-bold outline-none"></div><div class="mt-2 grid grid-cols-4 gap-1"><template x-for="v in [5,10,25,50]"><button @click="ante=v" :disabled="busy" class="rounded-lg bg-white/5 py-2 text-xs hover:bg-white/10 disabled:opacity-40" x-text="v+' EUR Demo'"></button></template></div><button @click="start" :disabled="busy||!canStart" class="mt-4 w-full rounded-xl bg-gradient-to-r from-emerald-400 to-cyan-400 py-3.5 font-black text-slate-950 disabled:opacity-40" x-text="busy?'Repartiendo…':'Repartir nueva mano'"></button></div></template>
                <template x-if="hand&&!finished"><div class="mt-4"><div class="rounded-xl border border-white/10 bg-black/20 p-3 text-sm"><div class="flex justify-between"><span class="text-slate-500">Ante</span><b x-text="money(hand.ante)"></b></div><div class="mt-2 flex justify-between"><span class="text-slate-500">Total en mesa</span><b class="text-amber-300" x-text="money(hand.wagered)"></b></div></div>
                    ...
                    <template x-if="hand.phase!=='preflop'"><div class="mt-4 space-y-3"><button @click="act('pasar')" :disabled="busy" class="w-full rounded-xl bg-cyan-400 py-3 font-black text-slate-950 disabled:opacity-40" x-text="checkLabel"></button><div class="rounded-xl border border-amber-300/15 bg-amber-300/5 p-3"><label class="text-[10px] font-black uppercase tracking-wider text-amber-200/70">Apuesta de esta calle (EUR Demo)</label><div class="mt-2 flex items-center rounded-lg border border-white/10 bg-black/20 px-3"><span>EUR Demo</span><input x-model.number="streetBet" :disabled="busy" type="number" min="1" max="1000" step="1" class="w-full bg-transparent px-2 py-2 font-bold outline-none"></div><div class="mt-2 grid grid-cols-3 gap-1"><template x-for="multiplier in [.5,1,2]"><button @click="streetBet=Math.max(1, hand.ante*multiplier)" :disabled="busy" class="rounded-lg bg-white/5 py-1.5 text-[10px] hover:bg-white/10" x-text="multiplier+'x ante'"></button></template></div><button @click="act('apostar')" :disabled="busy||!canStreetBet" class="mt-2 w-full rounded-lg bg-amber-400 py-2.5 font-black text-slate-950 disabled:opacity-40">Apostar <span x-text="money(streetBet)"></span> y continuar</button></div><button @click="act('retirarse')" :disabled="busy" class="w-full rounded-xl bg-white/5 py-2.5 text-sm text-slate-400 disabled:opacity-40">Retirarme de la mano</button></div></template>
                </div></template>
