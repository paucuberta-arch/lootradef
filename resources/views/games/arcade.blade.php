...
money(v){return new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR'}).format(Number(v||0))},card(v){return !v?'?':({11:'J',12:'Q',13:'K',14:'A'})[v]||v},toggleNumber(n){if(this.playing)return;if(this.numbers.includes(n))this.numbers=this.numbers.filter(x=>x!==n);else if(this.numbers.length<5)this.numbers.push(n)},async play(){if(this.playing)return;this.playing=true;this.result=null;this.error='';const requestToken=crypto.randomUUID();if(this.mode==='wheel')this.rotation+=1800+Math.random()*360;try{const r=await fetch(@js(route('games.originals.play',['game'=>$game['slug']])),{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content,'Accept':'application/json'},body:JSON.stringify({apuesta:this.apuesta,choice:this.choice,numbers:this.numbers,request_token:requestToken})});const d=await r.json();if(!r.ok)throw new Error(d.message||'No se pudo completar la partida.');await new Promise(x=>setTimeout(x,1600));this.result=d;this.saldo=Number(d.saldo);Alpine.store('wallet').saldo=this.saldo;window.dispatchEvent(new CustomEvent('saldo-updated',{detail:{saldo:this.saldo}}))}catch(e){this.error=e.message}this.playing=false}}}
</script>
@endpush
@endsection
