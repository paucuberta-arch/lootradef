...
<div class="max-w-[1200px] mx-auto px-4 sm:px-6 py-8" x-data="originalGame()">
    ...
    <section class="original-stage game-stage rounded-[2rem] border border-white/10 overflow-hidden p-4 sm:p-10" :class="playing && 'is-playing'">
        <div class="min-h-[280px] sm:min-h-[330px] grid place-items-center min-w-0">
            ...
            @elseif($game['mode'] === 'plinko')
                <div class="mx-auto max-w-[430px]">
                    ...
                    <div class="mt-2 grid grid-cols-11 gap-1 text-center text-[10px] font-bold text-cyan-300" aria-label="Multiplicadores"><span>12x</span><span>5x</span><span>2x</span><span>1.2x</span><span>.7x</span><span>.4x</span><span>.7x</span><span>1.2x</span><span>2x</span><span>5x</span><span>12x</span></div>
                </div>
            @elseif($game['mode'] === 'keno')
                <div class="w-full max-w-xl"><div class="grid grid-cols-5 sm:grid-cols-6 gap-2"><template x-for="n in 30"><button @click="toggleNumber(n)" class="w-full aspect-square rounded-lg border font-bold text-sm" :class="numbers.includes(n)?'bg-fuchsia-500 border-fuchsia-300':'bg-white/5 border-white/10'" x-text="n"></button></template></div><p class="text-center text-sm text-slate-500 mt-5">Elige exactamente 5 números · <span x-text="numbers.length"></span>/5</p><p class="mt-2 text-center text-xs text-slate-600">2 aciertos: x1,5 · 3: x31,4 · 4: x60 · 5: x500</p></div>
            ...
            @elseif($game['mode'] === 'baccarat')
                <div class="text-center w-full"><div class="flex justify-center gap-4 sm:gap-8"><div><p class="text-xs text-slate-500 mb-3">JUGADOR</p><div class="playing-card" x-text="result?.player ?? '?' "></div></div><div><p class="text-xs text-slate-500 mb-3">BANCA</p><div class="playing-card" x-text="result?.banker ?? '?' "></div></div></div><div class="flex flex-wrap gap-2 justify-center mt-7"><template x-for="c in [{id:'player',t:'Jugador 2,1333x'},{id:'banker',t:'Banca 2,1333x'},{id:'tie',t:'Empate 9,6x'}]"><button @click="choice=c.id" :class="choice===c.id?'bg-emerald-500 text-black':'bg-white/5'" class="px-4 sm:px-5 py-3 rounded-xl font-bold" x-text="c.t"></button></template></div></div>
            @else
        ...
        <div class="mx-auto mt-7 max-w-lg rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-4 text-center"><p class="text-[10px] font-black uppercase tracking-[.2em] text-emerald-300">Probabilidades publicadas</p><p class="mt-1 text-sm text-slate-300">RTP teórico <b class="text-emerald-300">{{ $game['rtp'] }}</b></p><p class="mt-1 text-xs text-slate-500">Es el retorno medio matemático a largo plazo; no garantiza el resultado de una partida concreta.</p></div>
