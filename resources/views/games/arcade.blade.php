...
@push('scripts')
<script>
function originalGame(){return{mode:@js($game['mode']),initialCard:@js($initialCard),saldo:{{ auth()->user()->saldo }},apuesta:2,choice:@js(match($game['mode']){'dice'=>'high','hilo'=>'higher','coin'=>'heads','baccarat'=>'player','nebula'=>'violet','mines'=>3,default=>null}),numbers:[],playing:false,result:null,error:'',rotation:0,reducedMotion:window.matchMedia('(prefers-reduced-motion: reduce)').matches,
money(v){return new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR'}).format(Number(v||0))},card(v){return !v?'?':({11:'J',12:'Q',13:'K',14:'A'})[v]||v},toggleNumber(n){if(this.playing)return;if(this.numbers.includes(n))this.numbers=this.numbers.filter(x=>x!==n);else if(this.numbers.length<5)this.numbers.push(n)},requestToken(){const webCrypto=globalThis.crypto;if(typeof webCrypto?.randomUUID==='function')return webCrypto.randomUUID();const bytes=new Uint8Array(16);webCrypto.getRandomValues(bytes);bytes[6]=(bytes[6]&15)|64;bytes[8]=(bytes[8]&63)|128;return Array.from(bytes,(b,i)=>([4,6,8,10].includes(i)?'-':'')+b.toString(16).padStart(2,'0')).join('')},async play(){if(this.playing)return;this.playing=true;this.result=null;this.error='';const requestToken=this.requestToken();if(this.mode==='wheel')this.rotation+=1800+Math.random()*360;try{const r=await fetch(@js(route('games.originals.play',['game'=>$game['slug']])),{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content,'Accept':'application/json'},body:JSON.stringify({apuesta:this.apuesta,choice:this.choice,numbers:this.numbers,request_token:requestToken})});const d=await r.json();if(!r.ok)throw new Error(d.message||'No se pudo completar la partida.');const revealDelay=this.reducedMotion?30:(this.mode==='wheel'?2200:800);await new Promise(x=>setTimeout(x,revealDelay));this.result=d;if(this.mode==='plinko'&&!this.reducedMotion)await new Promise(x=>setTimeout(x,1800));this.saldo=Number(d.saldo);Alpine.store('wallet').saldo=this.saldo;window.dispatchEvent(new CustomEvent('saldo-updated',{detail:{saldo:this.saldo}}))}catch(e){this.error=e.message}this.playing=false}}}
</script>
@endpush
@endsection
