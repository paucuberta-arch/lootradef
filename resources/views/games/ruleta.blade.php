...
<div class="mx-auto max-w-[1450px] px-4 py-7 sm:px-6 sm:py-10" x-data="rouletteGame()">
    <header class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex w-full flex-wrap gap-2 sm:w-auto"><a href="{{ route('games.roulette.european') }}" class="rounded-xl border px-3 py-2 text-xs font-bold {{ $variant === 'european' ? 'border-amber-400/40 bg-amber-400/10 text-amber-300' : 'border-white/10 text-slate-400' }}">Europea</a><a href="{{ route('games.roulette.lightning') }}" class="rounded-xl border px-3 py-2 text-xs font-bold {{ $variant === 'lightning' ? 'border-cyan-400/40 bg-cyan-400/10 text-cyan-300' : 'border-white/10 text-slate-400' }}">Lightning</a><span class="rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs text-emerald-200">RTP {{ $rtp }}</span><div class="rounded-xl border border-white/10 bg-white/5 px-3 sm:px-4 py-2 text-sm whitespace-nowrap"><span class="text-slate-500">Saldo</span> <b class="ml-1 text-brand-400" x-text="money(saldo)"></b></div></div>
    </header>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(390px,1.08fr)_minmax(420px,.92fr)]">
        ...
        <section class="roulette-console rounded-[1.75rem] border border-white/10 p-4 sm:p-7">
            ...
            <div class="mb-5 grid grid-cols-3 gap-2">
                ...
                <button @click="select('numero',0)" :class="selected('numero',0)?'ring-2 ring-white':''" class="rounded-xl bg-emerald-700 py-3 font-black">Cero <small class="block opacity-70">{{ $variant === 'lightning' ? 'x28,6 base' : 'x36 total' }}</small></button>
            </div>
            <div class="mb-3 flex items-center justify-between"><div><p class="text-[10px] font-black uppercase tracking-[.2em] text-fuchsia-400">Número exacto</p><h3 class="font-bold">Pulsa directamente del 1 al 36</h3></div><span class="rounded-lg bg-fuchsia-500/10 px-2 py-1 text-xs font-bold text-fuchsia-300">{{ $variant === 'lightning' ? 'Base x28,6 + boost' : 'Pago total x36' }}</span></div>
            <div class="grid grid-cols-6 gap-1.5 rounded-2xl border border-white/5 bg-black/20 p-3">
                <template x-for="n in 36" :key="n"><button @click="select('numero',n)" class="number-cell aspect-square rounded-lg border text-xs font-black" :class="numberClass(n)" :aria-label="'Apostar al número '+n" x-text="n"></button></template>
            </div>
        ...
        <aside class="grid gap-5 sm:grid-cols-2 lg:col-span-2 xl:grid-cols-[1.1fr_.9fr]">
            <section class="roulette-ticket rounded-2xl border border-amber-200/10 p-5"><p class="text-[10px] font-black uppercase tracking-[.2em] text-brand-400">Mesa privada · Paso 2</p><h2 class="mt-1 text-lg font-black">Confirma tu boleto</h2><div class="mt-4 rounded-xl border border-white/10 bg-black/20 p-3"><p class="text-xs text-slate-500">Tu selección</p><b class="mt-1 block text-amber-200" x-text="selectionLabel"></b></div><label class="mt-4 block text-xs text-slate-500" for="roulette-bet">Importe de la apuesta (EUR Demo)</label><div class="mt-2 flex items-center rounded-xl border border-white/10 bg-black/30 px-3"><span class="text-amber-300">EUR Demo</span><input id="roulette-bet" x-model.number="apuesta" type="number" min=".1" max="500" step=".1" :disabled="spinning" class="w-full bg-transparent px-2 py-3 font-bold outline-none"></div><div class="mt-3 grid grid-cols-4 gap-2"><template x-for="chip in [{value:1,tone:'ivory'},{value:5,tone:'red'},{value:10,tone:'blue'},{value:25,tone:'gold'}]" :key="chip.value"><button @click="apuesta=chip.value;window.lootraAudio?.play('select')" class="casino-chip-button aspect-square rounded-full text-xs font-black" :class="'casino-chip-button--'+chip.tone" x-text="chip.value+' EUR Demo'"></button></template></div><button @click="play" :disabled="spinning||apuesta>saldo||!apuesta" class="mt-4 w-full rounded-xl bg-gradient-to-r {{ $variant === 'lightning' ? 'from-cyan-300 via-indigo-300 to-violet-400' : 'from-[#f5dc92] via-[#d7a94f] to-[#a65d22]' }} py-4 font-black uppercase tracking-[.12em] text-slate-950 shadow-[0_12px_30px_rgba(214,170,79,.22)] transition hover:brightness-110 disabled:opacity-40" x-text="spinning?'La bola está girando…':'Girar ruleta'"></button><p x-show="error" class="mt-3 text-center text-xs text-red-300" x-text="error"></p></section>
...
function rouletteGame(){return{
    ...
    resultClass(c){return c==='rojo'?'bg-red-600 text-white':c==='negro'?'bg-slate-800 text-white border border-white/20':'bg-emerald-600 text-white'},money(v){return `${Number(v||0).toFixed(2)} EUR Demo`},
