            ...
            <section class="overflow-hidden rounded-3xl border border-white/10 bg-[#11182b] shadow-2xl shadow-black/30">
                ...
                <div class="p-5">
                    ...
                    <div x-show="selected" x-cloak>
                        ...
                        <div class="mt-2 flex items-center rounded-xl border border-white/10 bg-black/25 px-4"><span class="text-slate-500">€</span><input x-model.number="stake" type="number" min="1" max="5000" step="1" class="w-full bg-transparent px-3 py-3 font-bold outline-none" placeholder="10.00"></div>
                        <div class="mt-3 grid grid-cols-4 gap-2"><template x-for="amount in [5,10,25,50]"><button @click="stake=amount" class="rounded-lg bg-white/5 py-2 text-xs font-bold text-slate-400 hover:bg-white/10 hover:text-white" x-text="amount+'€'"></button></template></div>
...
function sportsbook() {
    return {
        ...
        select(match,choice){if(match.betting_open){this.selected={match,choice};this.message='';}}, money(v){return new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR'}).format(Number(v)||0)},
        ...
        async placeBet(){if(!this.selected||!this.stake)return;this.submitting=true;this.message='';try{const r=await fetch(`/apuestas/${this.selected.match.id}`,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body:JSON.stringify({seleccion:this.selected.choice.key,importe:this.stake})});const d=await r.json();if(!r.ok)throw new Error(d.message||'No se pudo realizar la apuesta.');this.$store.wallet.saldo=Number(d.balance);this.bets.unshift(d.bet);this.selected=null;this.message=d.message;this.error=false;}catch(e){this.message=e.message;this.error=true;}finally{this.submitting=false;} }
    }
}
</script>
@endsection
