...
<div x-show="{{ $name }}" x-cloak x-transition.opacity @keydown.escape.window="@if($closeable) {{ $name }} = false @endif" class="fixed inset-0 z-[120] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" role="presentation">
    ...
    <section role="dialog" aria-modal="true" aria-labelledby="{{ $name }}-title" {{ $attributes->class(['relative max-h-[calc(100dvh-2rem)] w-full overflow-y-auto rounded-2xl border border-white/10 bg-[#0F1626] p-4 shadow-2xl sm:p-6', $maxWidth]) }}>
